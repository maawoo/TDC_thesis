...
Both resulting rasters, visualizing the difference of median NDVI and VH backscatter relative to the summer period of 2017, show very similar patterns. Some extensive areas, such as in the northwest corner, show a significant decrease in both NDVI and VH backscatter. As can be seen in Figure @fig:roda_aoi these areas are usually not covered by forest, however, and are probably used for agriculture. 
...
The time-series plots confirm the reasoning behind selecting these particular pixels and show different developments of the values over time. While all datasets in plot B seem to be rather stable throughout the observed timeframe, a significant decrease of NDVI values and a noticeable decrease of VH backscatter values can be observed in plot A around May 2018. It should be noted that outlier values are likely to be present in the NDVI time-series, especially for winter months. A seasonal variation of the backscatter signal can be seen in plot B, which is particularly apparent for the descending dataset.
