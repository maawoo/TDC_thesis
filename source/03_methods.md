...
<!--
- Kurz klar stellen, dass die folgenden beschriebenen software components in das software tool ARDCube integriert wurden, welches in Section xy näher beschrieben wird...
-->
...
The feasibility to produce SAR backscatter products of an appropriate quality for time-series analysis with the SNAP software was investigated by @Truckenbrodt2019. The study particularly focused on producing Sentinel-1 ARD in regard to EODCs, and ultimately came to an affirming conclusion, which further supports the implementation into the software tool developed in the course of this work.
...
![ODC indexing workflow. A Python script is commonly used to generate dataset definition documents for each scene in a given EO dataset, whereas a product definition document needs to be created manually for the same dataset. All documents are stored in the YAML format. Adapted from [@ODC-Docs1].](source/figures/03_methods_2__odc.png){#fig:odc width=100% short-caption="ODC indexing workflow."}
...
- A user environment, which includes packages necessary for analyzing the data and allows being extended by users as required.
