...
- Creating a Digital Elevation Model (DEM) mosaic in the GeoTIFF format 
...
At first, the L1AS component is using provided parameters to query a cloud storage provider (Google Cloud Storage at the time of writing) and download any requested scenes. This workflow also checks for already existing scenes so only missing or new scenes are downloaded. A simple text file is used to track the scenes and enqueue them for subsequent processing.
