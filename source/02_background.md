...
To better understand the fundamentals of EODCs, Section @sec:concept provides a more comprehensive overview of the concept and how data cubes in EO are defined in literature. Section @sec:software-ecosystem on the other hand provides an overview of important software projects and the Open Data Cube initiative in particular. 
...
**gdalcubes**  
As the setup of most other EODCs is not trivial, the gdalcubes project [@Appel2019] aims to provide a solution that lets users concentrate on the analysis rather than data management. This is achieved by on-demand data cubes, which are only created when users perform computations on their data. As the name suggests, the widely-used Geospatial Data Abstraction Library (GDAL) is a major component and can handle a large variety of raster data formats [@Warmerdam2008]. The gdalcubes project is available as an open-source C++ library and an R package. 
